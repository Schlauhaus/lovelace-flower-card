import { LitElement, css, html } from "card-tools/src/lit-element";
...
class FlowerCard extends LitElement {
  ...
  const attribute = (icon, attr, min, max) => {
    ...
    const val = this.stateObj.attributes[attr];
      ...
      return html`
      <div class="attribute tooltip" data-tooltip="${val + " "+ unit + " | " + min + " ~ " + max + " " + unit}" @click="${() => moreInfo(this.stateObj.attributes.sensors[attr])}">
    ...
    return html`
    <ha-card>
      <div class="header" @click="${() => moreInfo(this.stateObj.entity_id)}">
